  ...
  const sendChatMessage = (message) => {
    if (onlineChatStore.logind) {
      if (interval === 0) {
        ...
        socket.send(JSON.stringify({ 'code': 201, 'name': onlineChatStore.username, 'userid': onlineChatStore.userid, 'message': message, 'channel': onlineChatStore.chatChannel, 'userqq': onlineChatStore.userqq }));
